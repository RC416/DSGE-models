...
for iteration in range(1,number_of_iterations):
    ...
    for kt0 in range(len(k_values)):         # for each level of starting capital...
        
        # Variables to store candidate optimal values for Value Function and Policy Function.
        v_max = float('-inf')
        kt1_optimal = 0
        
        for kt1 in range(len(k_values)):     # ...check all next period capital choices 
    
            # Calculate value of Value Function for given starting capital and next period capital choice.
            new_value_function_value = (log((k_values[kt0] ** alpha) - k_values[kt1] + (1-delta)*k_values[kt0])
                                        + beta*Value_Function[iteration-1, kt1])
