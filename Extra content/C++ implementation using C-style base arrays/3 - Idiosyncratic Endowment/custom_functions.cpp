	...
	double& V, double& g, int& g_index)
{
	...
	double* a_grid = params.a_grid;
	double* e_grid = params.e_grid;
	double* e_probs = params.e_probs;
	...
	for (int a_next_index = 0; a_next_index < number_of_a_values; a_next_index++)
	{
		...
		{
			future_period_payoff += Value_Function[a_next_index * number_of_e_values + e_prob_index] // Value_Function[a_next_index][e_prob_index]
				* e_probs[e_start_index * number_of_e_values + e_prob_index];						 // e_probs[e_start_index][e_prob_index]
		}
	...
	while ((dist > tolerance) && (iteration_count < max_iterations))
	{
		...
		for (int a_start_index = 0; a_start_index < number_of_a_values; a_start_index++)
		{
			for (int e_start_index = 0; e_start_index < number_of_e_values; e_start_index++)
			{
				...
				Value_Function_New[a_start_index * number_of_e_values + e_start_index] = V;
				Policy_Function_New[a_start_index * number_of_e_values + e_start_index] = g;
				Policy_Function_Index_New[a_start_index * number_of_e_values + e_start_index] = g_index;
			}
		}

		// Update search parameters.
		dist = GetDistanceArray2D(Value_Function, Value_Function_New, number_of_a_values, number_of_e_values);
		iteration_count++;

		// Update the Value Function and Policy Function.
		DeepCopyArray2D<double>(Value_Function_New, Value_Function, number_of_a_values, number_of_e_values);
		DeepCopyArray2D<double>(Policy_Function_New, Policy_Function, number_of_a_values, number_of_e_values);
		DeepCopyArray2D<int>(Policy_Function_Index_New, Policy_Function_Index, number_of_a_values, number_of_e_values);
	...
	double* Population_Distribution, double* Population_Distribution_New)
{
	...
	for (int i = 0; i < number_of_a_values; i++)
	{
		...
		{
			Population_Distribution[i * number_of_e_values + j] = uniform_density;
		}
	}
	...
	while ((dist > tolerance) && (iteration_count < max_iterations))
	{
		...
		for (int a_index = 0; a_index < number_of_a_values; a_index++)
		{
			...
			{
				// Track population inflow into the given state.
				double inflow = 0;
				...
				for (int a_index_in = 0; a_index_in < number_of_a_values; a_index_in++)
				{
					for (int e_index_in = 0; e_index_in < number_of_e_values; e_index_in++)
					{
						...
						if (Policy_Function_Index[a_index_in * number_of_e_values + e_index_in] == a_index)
						{
							// Calculate probability-weighted inflow.
							inflow += Population_Distribution[a_index_in * number_of_e_values + e_index_in] // Population_Distribution[a_index_in][e_index_in]
								* e_probs[e_index_in * number_of_e_values + e_index];						// e_probs[e_index_in][e_index]
						}
					}
				}
				// Assign total inflow to the Population Distribution.
				Population_Distribution_New[a_index * number_of_e_values + e_index] = inflow;
			}
		}

		// Update search parameters.
		dist = GetDistanceArray2D(Population_Distribution_New, Population_Distribution, number_of_a_values, number_of_e_values);
		iteration_count++;

		// Update the Population Distribution.
		DeepCopyArray2D<double>(Population_Distribution_New, Population_Distribution, number_of_a_values, number_of_e_values);
	...
	int n_rows, int n_cols)
{
	...
	for (int i = 0; i < n_rows; i++)
	{
		...
		{
			mcc += Population_Distribution[i * n_cols + j] * Policy_Function[i * n_cols + j];
		}
	}
	return mcc;
}
	...
	int n_rows, int n_cols)
{
	...
	for (int i = 0; i < n_rows; i++)
	{
		for (int j = 0; j < n_cols; j++)
		{
			...
			element_diff = abs(Array1[i * n_cols + j] - Array2[i * n_cols + j]);
	...
	int n_rows, int n_cols)
{
	...
	for (int i = 0; i < n_rows; i++)
	{
		for (int j = 0; j < n_cols; j++)
		{
			Array_Copy[i * n_cols + j] = Array_Original[i * n_cols + j];
		}
	}
}
...
template<class number_type>
void WriteArrayToCSV(number_type* Array, const char* file_name, int n_rows, int n_cols)
{
...
template void WriteArrayToCSV<double>(double* Array, const char* file_name, int n_rows, int n_cols);